...
function leastInterval(tasks, n) {
    ...
    let taskList = [...map.values()].sort((x, y) => y - x)
    let max = taskList.lastIndexOf(taskList[0])
    return Math.max(tasks.length, (taskList[0] - 1) * (n + 1) + max + 1)
}

let tasks = ["A", "A", "A", "B", "B", "B"]
let n = 2
let ans = leastInterval(tasks, n)
console.log(ans)